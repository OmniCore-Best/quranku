...
export default function HadistDetailPage() {
  ...
  const hadithNumber = searchParams.get('number'); // untuk mode spesifik
  ...
  // Setup intersection observer untuk infinite scroll (hanya untuk mode daftar)
  useEffect(() => {
    if (isSingleMode || loading) return;
  ...
  }, [isSingleMode, loading, hasMore, loadingMore, isOnline, allHadiths]);

  // Fungsi mengambil data awal (daftar) – sama seperti sebelumnya
  ...
  // Fungsi memuat lebih banyak hadis (infinite scroll)
  ...
  // Fungsi mengambil satu hadis spesifik
  const fetchSingleHadith = async (number: number) => {
    try {
      setLoading(true);
      setError(null);
      setIsSingleMode(true);

      // Coba ambil dari cache terlebih dahulu (opsional, perlu implementasi db.getSingleHadith)
      // Di sini kita asumsikan belum ada, jadi langsung fetch online

      if (!isOnline) {
        setError('Anda sedang offline. Tidak dapat memuat hadis spesifik.');
        setLoading(false);
        return;
      }
      ...
      if (data.code === 200 && data.data.contents) {
        ...
        // Opsional: simpan ke cache untuk akses offline nanti
        // await db.saveSingleHadith(bookId, hadithData);
      } else {
  ...
  // Fungsi share hadis dengan penanganan user gesture
  const handleShareHadith = async (hadith: Hadith) => {
    ...
    try {
      ...
      // Fungsi fallback: unduh gambar dan salin teks
      ...
      // Cek dukungan share dengan file
        ...
        try {
          ...
          if (shareError instanceof DOMException) {
            if (shareError.message.includes('user gesture')) {
              // Fallback karena butuh user gesture
              await fallback();
      ...
      // Fallback ke share teks saja jika bisa
      ...
      // Tidak ada dukungan share sama sekali
      else {
        await fallback();
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen p-4">
      <div className="max-w-4xl mx-auto">
        ...
        {isSingleMode && singleHadith && (
          ...
          >
            ...
            <div className="mb-4 text-right">
              <p className="font-arabic text-2xl leading-loose text-gray-900">{singleHadith.arab}</p>
            </div>
        ...
        {/* Mode Daftar Hadis (Infinite Scroll) */}
        {!isSingleMode && (
          <>
            <motion.div
              className="space-y-4"
