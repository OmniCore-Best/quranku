...
// ----------------------------------------------------------------------
// System prompt untuk LLM (lengkapi sesuai kebutuhan)
// ----------------------------------------------------------------------
  ...
  <ReactMarkdown
    ...
    components={{
      ...
      a: ({ href, children }) => {
        ...
        return (
          <a href={href} onClick={(e) => { e.preventDefault(); onNavigate(href); }} className="text-blue-600 underline font-medium">
            {children}
          </a>
        );
      },
...
export default function QurankuAIPage() {
  ...
  const getWelcomeMessage = (): ChatMessage => ({
    id: 'welcome',
    role: 'assistant',
    content: 'Halo! Saya asisten cerdas Quranku...',
    timestamp: new Date(),
  });
  ...
  const sendChatMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: (Date.now() + 1).toString(), role: 'assistant', content: 'Maaf, layanan AI sedang sibuk.', timestamp: new Date() }]);
      setStatus('offline');
    } finally {
      setIsLoading(false);
    }
  ...
  const captureElementWithWatermark = async (element: HTMLElement): Promise<Blob> => {
    ...
    const dataUrl = await toPng(clone, { quality: 0.95, pixelRatio: 2, backgroundColor: '#ffffff', cacheBust: true });
  ...
  const renderSearchResultCard = (result: SearchResult, idx: number) => {
    ...
    const relevansiColor = relevansi === 'tinggi' ? 'bg-emerald-100 text-emerald-800' : relevansi === 'sedang' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-600';
      ...
      return (
        <div id={uniqueId} key={idx} className="bg-white rounded-2xl p-4 mb-3 shadow-sm border border-gray-100 relative">
          ...
          <div className="flex items-center gap-2 mb-2 pr-10">
            <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs">Ayat</span>
          ...
          <button onClick={() => router.push(`/surah/${data.id_surat}?ayat=${data.nomor_ayat}`)} className="mt-3 text-xs text-blue-600 flex items-center gap-1">Baca lengkap <FaExternalLinkAlt className="w-3 h-3" /></button>
        </div>
      );
    }
    if (tipe === 'tafsir') {
      return (
        <div id={uniqueId} key={idx} className="bg-white rounded-2xl p-4 mb-3 shadow-sm border border-gray-100 relative">
          <button onClick={() => handleDownloadResult(uniqueId, downloadTitle)} disabled={downloadingId === uniqueId} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center">
            {downloadingId === uniqueId ? <FaSpinner className="animate-spin w-4 h-4" /> : <FaShareAlt className="w-4 h-4 text-gray-600" />}
          </button>
          <div className="flex items-center gap-2 mb-2 pr-10">
          ...
          <button onClick={() => router.push(`/surah/${data.id_surat}?ayat=${data.nomor_ayat}`)} className="mt-2 text-xs text-blue-600">Lihat Ayat</button>
        </div>
      );
    }
    if (tipe === 'doa') {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] flex flex-col">
      ...
      <div className="h-14 px-4 flex items-center justify-between bg-white border-b border-gray-100 sticky top-0 z-20">
        <button className="w-8 h-8 rounded-full hover:bg-gray-100"><FaBars className="w-5 h-5 text-gray-700" /></button>
        <div className="flex items-center gap-2">
          ...
          {mode === 'chat' && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
              status === 'online' ? 'bg-green-100 text-green-700' : status === 'fallback' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
            }`}>
              {status === 'online' ? 'Online' : status === 'fallback' ? 'Backup' : 'Offline'}
            </span>
          )}
        </div>
        <button onClick={mode === 'chat' ? startNewChat : () => setSearchResults([])} className="w-8 h-8 rounded-full hover:bg-gray-100"><FaPlus className="w-5 h-5 text-gray-700" /></button>
      </div>

      {/* Chat area / hasil semantic */}
      <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-[160px]">
        {mode === 'chat' ? (
          <>
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center mr-2 mt-1"><FaRobot className="w-4 h-4 text-white" /></div>}
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${msg.role === 'user' ? 'bg-[#4A8CFF] text-white rounded-br-md' : 'bg-white text-gray-800 shadow-sm rounded-bl-md'}`}>
                  {msg.role === 'assistant' ? <MarkdownContent content={msg.content} onNavigate={handleNavigate} /> : <p className="whitespace-pre-wrap break-words">{msg.content}</p>}
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center mr-2"><FaSpinner className="animate-spin text-white w-4 h-4" /></div>
                <div className="bg-white rounded-2xl rounded-bl-md px-4 py-2.5 text-sm text-gray-500 shadow-sm">AI sedang mengetik...</div>
              </div>
            )}
          ...
          <div>
            {searchLoading && <div className="text-center py-8"><FaSpinner className="animate-spin w-6 h-6 text-blue-500 mx-auto mb-2" /><p className="text-sm text-gray-500">Mencari makna...</p></div>}
            ...
            {!searchLoading && searchResults.length > 0 && <div>{searchResults.map((res, i) => renderSearchResultCard(res, i))}</div>}
            {!searchLoading && searchQuery && searchResults.length === 0 && !searchError && <div className="text-center py-8 text-gray-500"><FaSearch className="w-8 h-8 mx-auto mb-2 text-gray-300" /><p className="text-sm">Tidak ada hasil untuk "{searchQuery}"</p></div>}
      ...
      {/* Input area fixed di atas bottom navbar */}
      <div className="fixed left-0 right-0 bottom-[70px] z-20 bg-transparent px-4 py-3">
        <div className="bg-white rounded-3xl shadow-md border border-gray-100 p-3">
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <div className="flex gap-2">
              <button className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200"><FaMicrophone className="w-4 h-4 text-gray-600" /></button>
              <button onClick={handleSubmit} disabled={(mode === 'chat' && (!currentInputValue.trim() || isLoading)) || (mode === 'semantic' && (!currentInputValue.trim() || searchLoading))} className="w-9 h-9 rounded-full bg-blue-500 text-white flex items-center justify-center hover:bg-blue-600 disabled:opacity-40 transition">
                {mode === 'chat' ? <FaPaperPlane className="w-4 h-4" /> : <FaSearch className="w-4 h-4" />}
              </button>
            </div>
          </div>
        </div>
        <p className="text-center text-[10px] text-gray-400 mt-2">{mode === 'chat' ? 'Enter kirim, Shift+Enter baris baru' : 'Enter cari'}</p>
      </div>
    </div>
  );
}