...
export default function QurankuAIPage() {
  ...
  // ----------------------------------------------------------------------
  // Capture PNG dengan html2canvas – tangani warna oklch via onclone
  // ----------------------------------------------------------------------
  const captureElementWithWatermark = async (element: HTMLElement): Promise<Blob> => {
    // Fungsi untuk menerapkan warna solid dari elemen asli ke clone
    const applyComputedColors = (clonedDoc: Document, originalEl: HTMLElement) => {
      // Beri tanda pada elemen asli agar bisa ditemukan di clone
      const uniqueAttr = 'data-quranku-clone-id';
      const id = Date.now().toString();
      originalEl.setAttribute(uniqueAttr, id);

      // Cari elemen yang sama di dokumen clone
      const cloneTarget = clonedDoc.querySelector(`[${uniqueAttr}="${id}"]`) as HTMLElement;
      if (!cloneTarget) {
        originalEl.removeAttribute(uniqueAttr);
        return;
      }

      // Properti warna yang mungkin mengandung oklch
      const colorProps = [
        'color',
        'backgroundColor',
        'borderColor',
      ...
      const origStyles = window.getComputedStyle(originalEl);

      colorProps.forEach((prop) => {
        const val = origStyles.getPropertyValue(prop);
        if (val && val.includes('oklch')) {
          cloneTarget.style.setProperty(prop, val, 'important');
        }
      });

      // Tangani semua anak secara rekursif
      const origChildren = originalEl.querySelectorAll('*');
      const cloneChildren = cloneTarget.querySelectorAll('*');

      origChildren.forEach((origChild, index) => {
        const cloneChild = cloneChildren[index];
        if (!cloneChild) return;

        const childStyles = window.getComputedStyle(origChild);
        colorProps.forEach((prop) => {
          const val = childStyles.getPropertyValue(prop);
          if (val && val.includes('oklch')) {
            (cloneChild as HTMLElement).style.setProperty(prop, val, 'important');
          }
        });
      });

      // Bersihkan atribut sementara
      originalEl.removeAttribute(uniqueAttr);
    };
    ...
    const canvas = await html2canvas(element, {
      ...
      logging: false,
      onclone: (clonedDoc) => {
        applyComputedColors(clonedDoc, element);
      },
    });
